...
app.post("/get_addresses", async (req, res) => {
  ...
  // Get city of original address
  console.log("Finding city of original address");
  const coordinate_address = await getAddress(
    lat.toString(),
    lon.toString()
  );
  const city = JSON.parse(coordinate_address).address.city;
  console.log(city);
  console.log("\n");
  await timeout(750);
  ...
  return res.send({ addresses: addresses, city: city });
});
...
app.post("/rideshare_bots", async (req, res) => {
  ...
  let options = {
    uri: `http://${SERVER_IP}:${config.ridesharePort}/get_addresses`,
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  options = {
    uri: `http://${SERVER_IP}:${config.ridesharePort}/uber_bot`,
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  options = {
    uri: `http://${SERVER_IP}:${config.ridesharePort}/lyft_bot`,
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  Promise.all([averages[0], averages[1]]).then((averages) => {
    ...
    */
    res.send({ uber: averages[0], lyft: averages[1] });
  });
});
...
app.post("/lyft_bot", async (req, res) => {
  ...
  for (let i = 1; i < ADDRESSES.length; i++) {
    await page.type("input[name='fare-end']", ADDRESSES[i]);
    await page.click(
      "button[class='BaseButton-sc-174wt3j-0 secondaryButton-sc-174wt3j-5 hDYCeO']"
    );
    await page.waitFor(5000);
    ...
    if (
      ...
      if ((await page.$("td[class='_2B8KEJ _3EfUeQ _2eaC_M']")) !== null) {
        ...
        console.log("Lyft: " + avg.toFixed(2));
        console.log("\n");
      }
    }
    const button = await page.$("input[name='fare-end']");
    await button.click();
    await timeout(1000);
    await button.click({ clickCount: 3 });
  }
